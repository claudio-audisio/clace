...
class TestParams {
public:
    TestParams(string fenGame, bool white, list<Position>* expectedPositions) {
...
TEST_P(GetKingAttacksTest, getKingAttacksTest) {
    ...
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getKingAttacks(params->white), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    GetKingAttacksTest,
    ::testing::Values(
        new TestParams("8/8/8/3K4/8/8/8/8 w - - 0 1", true, new list<Position>{ 18, 19, 20, 26, 28, 34, 35, 36 }),
        new TestParams("8/8/8/8/8/5k2/8/8 w - - 0 1", false, new list<Position>{ 36, 37, 38, 44, 46, 52, 53, 54 }),
        new TestParams("K7/8/8/8/8/8/8/8 w - - 0 1", true, new list<Position>{ 1, 8, 9 }),
        new TestParams("8/8/8/8/8/8/6k1/8 w - - 0 1", false, new list<Position>{ 45, 46, 47, 53, 55, 61, 62, 63 }),
        new TestParams("8/8/1p6/K7/1P6/8/8/8 w - - 0 1", true, new list<Position>{ 16, 17, 25, 32 }),
        new TestParams("8/8/8/7P/6pk/8/8/8 w - - 0 1", false, new list<Position>{ 30, 31, 46, 47 })
    )
);
...
TEST_P(GetKingMovesTest, getKingMovesTest) {
    TestParams* params = GetParam();
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getKingMoves(params->white, game->getCastlingInfo()), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        new TestParams("8/8/1p6/K7/1P6/8/8/8 w - - 0 1", true, new list<Position>{16, 17, 25, 32}),
        new TestParams("8/8/8/7P/6pk/8/8/8 w - - 0 1", false, new list<Position>{30, 31, 46, 47}),
        new TestParams("4k3/8/8/8/8/8/8/8 w KQkq - 0 1", false, new list<Position>{2, 3, 11, 12, 13, 5, 6}),
        new TestParams("8/8/8/8/8/8/8/4K3 w KQkq - 0 1", true, new list<Position>{58, 59, 51, 52, 53, 61, 62}),
        new TestParams("4k3/8/8/8/8/8/8/8 w KQk - 0 1", false, new list<Position>{3, 11, 12, 13, 5, 6}),
        new TestParams("8/8/8/8/8/8/8/4K3 w Kkq - 0 1", true, new list<Position>{59, 51, 52, 53, 61, 62}),
        new TestParams("4k3/8/8/8/8/8/8/8 w KQq - 0 1", false, new list<Position>{2, 3, 11, 12, 13, 5}),
        new TestParams("8/8/8/8/8/8/8/4K3 w Qkq - 0 1", true, new list<Position>{58, 59, 51, 52, 53, 61}),
        new TestParams("4k3/8/8/8/8/8/8/8 w KQ - 0 1", false, new list<Position>{3, 11, 12, 13, 5}),
        new TestParams("8/8/8/8/8/8/8/4K3 w kq - 0 1", true, new list<Position>{ 59, 51, 52, 53, 61 })
    )
);
...
TEST_P(GetQueenAttacksTest, getQueenAttacksTest) {
    TestParams* params = GetParam();
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getQueenAttacks(params->white), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    GetQueenAttacksTest,
    ::testing::Values(
        new TestParams("8/8/8/3Q4/8/8/8/8 w - - 0 1", true, new list<Position>{0, 9, 18, 36, 45, 54, 63, 6, 13, 20, 34, 41, 48, 19, 11, 3, 35, 43, 51, 59, 24, 25, 26, 28, 29, 30, 31}),
        new TestParams("8/8/8/8/8/5q2/8/8 w - - 0 1", false, new list<Position>{ 0, 9, 18, 27, 36, 54, 63, 38, 31, 52, 59, 37, 29, 21, 13, 5, 53, 61, 40, 41, 42, 43, 44, 46, 47 }),
        new TestParams("Q7/8/8/8/8/8/8/8 w - - 0 1", true, new list<Position>{ 9, 18, 27, 36, 45, 54, 63, 8, 16, 24, 32, 40, 48, 56, 1, 2, 3, 4, 5, 6, 7 }),
        new TestParams("8/8/8/8/8/8/6q1/8 w - - 0 1", false, new list<Position>{ 0, 9, 18, 27, 36, 45, 63, 61, 47, 46, 38, 30, 22, 14, 6, 62, 48, 49, 50, 51, 52, 53, 55 }),
        new TestParams("8/2p5/8/Q7/8/2P5/P7/8 w - - 0 1", true, new list<Position>{ 33, 17, 10, 25, 26, 27, 28, 29, 30, 31, 32, 40, 0, 8, 16 }),
        new TestParams("8/8/5P1P/8/7q/8/5p2/8 w - - 0 1", false, new list<Position>{ 46, 30, 21, 32, 33, 34, 35, 36, 37, 38, 63, 55, 47, 31, 23 })
    )
);
...
TEST_P(GetRookAttacksTest, getRookAttacksTest) {
    TestParams* params = GetParam();
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getRookAttacks(params->white), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    GetRookAttacksTest,
    ::testing::Values(
        new TestParams("8/8/1R6/8/8/8/8/8 w - - 0 1", true, new list<Position>{ 16, 18, 19, 20, 21, 22, 23, 1, 9, 25, 33, 41, 49, 57 }),
        new TestParams("8/8/8/8/8/8/6r1/8 w - - 0 1", false, new list<Position>{ 48, 49, 50, 51, 52, 53, 55, 62, 46, 38, 30, 22, 14, 6 }),
        new TestParams("RP6/P7/8/8/8/8/8/8 w - - 0 1", true, new list<Position>()),
        new TestParams("8/8/8/8/8/8/7p/6pr w - - 0 1", false, new list<Position>()),
        new TestParams("8/8/1R1P4/8/1p6/8/8/8 w - - 0 1", true, new list<Position>{ 16, 18, 1, 9, 25, 33 }),
        new TestParams("8/8/8/8/6p1/8/4P1r1/8 w - - 0 1", false, new list<Position>{ 52, 53, 55, 62, 46 })
    )
);
...
TEST_P(GetBishopAttacksTest, getBishopAttacksTest) {
    TestParams* params = GetParam();
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getBishopAttacks(params->white), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        new TestParams("8/8/8/3B4/8/8/8/8 w - - 0 1", true, new list<Position>{0, 9, 18, 36, 45, 54, 63, 6, 13, 20, 34, 41, 48}),
        new TestParams("8/8/8/8/8/5b2/8/8 w - - 0 1", false, new list<Position>{0, 9, 18, 27, 36, 54, 63, 38, 31, 52, 59}),
        new TestParams("B7/8/8/8/8/8/8/8 w - - 0 1", true, new list<Position>{9, 18, 27, 36, 45, 54, 63}),
        new TestParams("8/8/8/8/8/8/6b1/8 w - - 0 1", false, new list<Position>{0, 9, 18, 27, 36, 45, 63, 61, 47}),
        new TestParams("8/2p5/8/B7/8/2P5/8/8 w - - 0 1", true, new list<Position>{33, 17, 10}),
        new TestParams("8/8/5P2/8/7b/8/5p2/8 w - - 0 1", false, new list<Position>{46, 30, 21})
    )
);
...
TEST_P(GetKnightAttacksTest, getKnightAttacksTest) {
    TestParams* params = GetParam();
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getKnightAttacks(params->white), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        new TestParams("8/8/2N5/8/8/8/8/8 w - - 0 1", true, new list<Position>{ 1, 8, 24, 33, 35, 28, 3, 12 }),
        new TestParams("8/8/8/8/3n4/8/8/8 w - - 0 1", false, new list<Position>{ 20, 18, 25, 29, 41, 45, 50, 52 }),
        new TestParams("N7/8/8/8/8/8/8/8 w - - 0 1", true, new list<Position>{ 10, 17 }),
        new TestParams("8/8/8/8/8/8/8/n7 w - - 0 1", false, new list<Position>{ 41, 50 }),
        new TestParams("1P6/P7/2N5/8/1p1p4/8/8/8 w - - 0 1", true, new list<Position>{ 24, 33, 35, 28, 3, 12 }),
        new TestParams("8/8/8/8/3n4/1P3P2/2p1p3/8 w - - 0 1", false, new list<Position>{ 20, 18, 25, 29, 41, 45 })
    )
);
...
TEST_P(GetPawnMovesTest, getPawnMovesTest) {
    TestParams* params = GetParam();
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getPawnMoves(params->white, game->getEnPassantPosition()), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    GetPawnMovesTest,
    ::testing::Values(
        new TestParams("8/p7/8/8/8/8/8/8 w - - 0 1", false, new list<Position>{ 16, 24 }),
        new TestParams("8/8/p7/8/8/8/8/8 w - - 0 1", false, new list<Position>{ 24 }),
        new TestParams("8/8/8/8/8/8/P7/8 w - - 0 1", true, new list<Position>{ 40, 32 }),
        new TestParams("8/8/8/8/8/P7/8/8 w - - 0 1", true, new list<Position>{ 32 }),
        new TestParams("8/8/8/3p4/3P4/8/8/8 w - - 0 1", false, new list<Position>()),
        new TestParams("8/8/8/3p4/3P4/8/8/8 w - - 0 1", true, new list<Position>()),
        new TestParams("8/8/8/3p4/2P1b3/8/8/8 w - - 0 1", false, new list<Position>{ 34, 35 }),
        new TestParams("8/8/8/2B1p3/3P4/8/8/8 w - - 0 1", true, new list<Position>{ 27, 28 }),
        new TestParams("8/8/8/pP6/8/8/8/8 w - a6 0 1", true, new list<Position>{ 16, 17 }),
        new TestParams("8/8/8/pP5P/8/8/8/8 w - a6 0 1", true, new list<Position>{ 16, 17, 23 }),
        new TestParams("8/8/8/8/6Pp/8/8/8 w - g3 0 1", false, new list<Position>{ 46, 47 }),
        new TestParams("8/8/8/8/p5Pp/8/8/8 w - g3 0 1", false, new list<Position>{ 40, 46, 47 })
    )
);
...
class TestParams2 {
public:
    TestParams2(string fenGame, bool white, Position position, list<Position>* expectedPositions) {
        this->fenGame = fenGame;
        this->white = white;
...
TEST_P(GetSinglePawnMovesTest, getSinglePawnMovesTest) {
    ...
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getPawnMoves(params->position, params->white, game->getEnPassantPosition()), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        new TestParams2("8/6bb/8/8/R1pP2k1/4P3/P7/K7 w - d3 0 1", false, 34, new list<Position>{ 42, 43 }),						// Following from https://www.chessprogramming.org/En_passant
        new TestParams2("2r3k1/1q1nbppp/r3p3/3pP3/pPpP4/P1Q2N2/2RN1PPP/2R4K w - b3 0 1", false, 32, new list<Position>{ 41 }),
        new TestParams2("2r3k1/1q1nbppp/r3p3/3pP3/pPpP4/P1Q2N2/2RN1PPP/2R4K w - b3 0 1", false, 34, new list<Position>{ 41 }),
        new TestParams2("rnbqkbnr/ppppppp1/8/7p/P7/8/1PPPPPPP/RNBQKBNR w - h6 0 1", true, 32, new list<Position>{ 24 })
    )
);
...
TEST_P(GetPawnAttacksTest, getPawnAttacksTest) {
    TestParams* params = GetParam();
    Game* game = FEN::fenToNewGame(params->fenGame);
    GTEST_ASSERT_TRUE(checkBoard(game->getBoard().getPawnAttacks(params->white), *params->expectedPositions));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    GetPawnAttacksTest,
    ::testing::Values(
        new TestParams("8/p7/8/8/8/8/8/8 w - - 0 1", false, new list<Position>{ 17 }),
        new TestParams("8/8/p7/8/8/8/8/8 w - - 0 1", false, new list<Position>{ 25 }),
        new TestParams("8/8/8/8/8/8/P7/8 w - - 0 1", true, new list<Position>{ 41 }),
        new TestParams("8/8/8/8/8/P7/8/8 w - - 0 1", true, new list<Position>{ 33 }),
        new TestParams("8/8/8/3p4/3P4/8/8/8 w - - 0 1", false, new list<Position>{ 34, 36 }),
        new TestParams("8/8/8/3p4/3P4/8/8/8 w - - 0 1", true, new list<Position>{ 26, 28 }),
        new TestParams("8/8/8/3p4/2P1b3/8/8/8 w - - 0 1", false, new list<Position>{ 34 }),
        new TestParams("8/8/8/2B1p3/3P4/8/8/8 w - - 0 1", true, new list<Position>{ 28 })
    )
);


TEST(BoardTest, getPawnMovesTest) {
    Board board;
    board.setPiece(49, WPawn);
    board.setPiece(40, BPawn);
    board.setPiece(42, BPawn);
    GTEST_ASSERT_TRUE(checkBoard(board.getPawnMoves(49, true, 0), 40, 41, 42, 33));
    EXPECT_EQ(board.getPawnMoves(49, true, 0), 0x70200000000LL);
    board.setPiece(33, BPawn);   // disable double push
    EXPECT_EQ(board.getPawnMoves(49, true, 0), 0x70000000000LL);
    board.setPiece(41, BPawn);
    EXPECT_EQ(board.getPawnMoves(49, true, 0), 0x50000000000LL);
    board.setPiece(40, WPawn);
    EXPECT_EQ(board.getPawnMoves(49, true, 0), 0x40000000000LL);

    board.setPiece(9, BPawn);
    board.setPiece(16, WPawn);
    board.setPiece(18, WPawn);
    EXPECT_EQ(board.getPawnMoves(9, false, 0), 0x2070000LL);
    board.setPiece(25, WPawn);   // disable double push
    EXPECT_EQ(board.getPawnMoves(9, false, 0), 0x70000LL);
    board.setPiece(17, WPawn);
    EXPECT_EQ(board.getPawnMoves(9, false, 0), 0x50000LL);
    board.setPiece(16, BPawn);
    EXPECT_EQ(board.getPawnMoves(9, false, 0), 0x40000LL);

    board.setPiece(0, WPawn);
    EXPECT_EQ(board.getPawnMoves(0, true, 0), 0x0LL);
    board.setPiece(63, BPawn);
    GTEST_ASSERT_TRUE(checkBoardNoPos(board.getPawnMoves(63, false, 0)));
    EXPECT_EQ(board.getPawnMoves(63, false, 0), 0x0LL);

    board.reset();

    // EnPassant
    EXPECT_EQ(board.getPawnMoves(24, true, 18), 0x10000LL);
    EXPECT_EQ(board.getPawnMoves(24, true, 17), 0x30000LL);
    EXPECT_EQ(board.getPawnMoves(25, true, 18), 0x60000LL);
    EXPECT_EQ(board.getPawnMoves(25, true, 16), 0x30000LL);
    EXPECT_EQ(board.getPawnMoves(32, false, 42), 0x10000000000LL);
    EXPECT_EQ(board.getPawnMoves(32, false, 41), 0x30000000000LL);
    EXPECT_EQ(board.getPawnMoves(33, false, 42), 0x60000000000LL);
    EXPECT_EQ(board.getPawnMoves(33, false, 40), 0x30000000000LL);
}

TEST(BoardTest, getKnightPositionsTest) {
    Board board;
    EXPECT_EQ(board.getKnightMoves(0, false), 0x20400LL);
    EXPECT_EQ(board.getKnightMoves(7, false), 0x402000LL);
    EXPECT_EQ(board.getKnightMoves(63, false), 0x20400000000000LL);
    EXPECT_EQ(board.getKnightMoves(56, false), 0x4020000000000LL);
    EXPECT_EQ(board.getKnightMoves(45, false), 0x5088008850000000LL);
    board.setPiece(55, BPawn);
    EXPECT_EQ(board.getKnightMoves(45, true), 0x5088008850000000LL);
    EXPECT_EQ(board.getKnightMoves(45, false), 0x5008008850000000LL);
}

TEST(BoardTest, getBishopPositionsTest) {
    Board board;
    EXPECT_EQ(board.getBishopMoves(0, false), 0x8040201008040200LL);
    EXPECT_EQ(board.getBishopMoves(7, false), 0x102040810204000LL);
    EXPECT_EQ(board.getBishopMoves(63, false), 0x40201008040201LL);
    EXPECT_EQ(board.getBishopMoves(56, false), 0x2040810204080LL);
    EXPECT_EQ(board.getBishopMoves(54, false), 0xa000a01008040201LL);
    board.setPiece(27, BPawn);
    EXPECT_EQ(board.getBishopMoves(54, true), 0xa000a01008000000LL);
    EXPECT_EQ(board.getBishopMoves(54, false), 0xa000a01000000000LL);

}

TEST(BoardTest, getRookPositionsTest) {
    Board board;
    EXPECT_EQ(board.getRookMoves(0, false), 0x1010101010101feLL);
    EXPECT_EQ(board.getRookMoves(7, false), 0x808080808080807fLL);
    EXPECT_EQ(board.getRookMoves(63, false), 0x7f80808080808080LL);
    EXPECT_EQ(board.getRookMoves(56, false), 0xfe01010101010101LL);
    EXPECT_EQ(board.getRookMoves(54, false), 0x40bf404040404040LL);
    board.setPiece(30, BPawn);
    EXPECT_EQ(board.getRookMoves(54, true), 0x40bf404040000000LL);
    EXPECT_EQ(board.getRookMoves(54, false), 0x40bf404000000000LL);

}

TEST(BoardTest, getQueenPositionsTest) {
    Board board;
    EXPECT_EQ(board.getQueenMoves(0, false), 0x81412111090503feLL);
    EXPECT_EQ(board.getQueenMoves(7, false), 0x8182848890a0c07fLL);
    EXPECT_EQ(board.getQueenMoves(63, false), 0x7fc0a09088848281LL);
    EXPECT_EQ(board.getQueenMoves(56, false), 0xfe03050911214181LL);
    EXPECT_EQ(board.getQueenMoves(54, false), 0xe0bfe05048444241LL);
    board.setPiece(51, BPawn);
    EXPECT_EQ(board.getQueenMoves(54, true), 0xe0b8e05048444241LL);
    EXPECT_EQ(board.getQueenMoves(54, false), 0xe0b0e05048444241LL);
}

TEST(BoardTest, getKingPositionTest) {
    Board board;
    EXPECT_EQ(board.getKingMoves(0, false, (CastlingInfo)0b1111), 0x302LL);
    EXPECT_EQ(board.getKingMoves(7, false, (CastlingInfo)0b1111), 0xc040LL);
    EXPECT_EQ(board.getKingMoves(63, false, (CastlingInfo)0b1111), 0x40c0000000000000LL);
    EXPECT_EQ(board.getKingMoves(56, false, (CastlingInfo)0b1111), 0x203000000000000LL);
    EXPECT_EQ(board.getKingMoves(54, false, (CastlingInfo)0b1111), 0xe0a0e00000000000LL);
    board.setPiece(62, BPawn);
    EXPECT_EQ(board.getKingMoves(54, false, (CastlingInfo)0b1111), 0xa0a0e00000000000LL);
    EXPECT_EQ(board.getKingMoves(54, true, (CastlingInfo)0b1111), 0xe0a0e00000000000LL);

    // castling
    EXPECT_EQ(board.getKingMoves(4, false, (CastlingInfo)0b1111), 0x386cLL);
    EXPECT_EQ(board.getKingMoves(4, false, (CastlingInfo)0b1110), 0x3868LL);
    EXPECT_EQ(board.getKingMoves(4, false, (CastlingInfo)0b1100), 0x3828LL);
    EXPECT_EQ(board.getKingMoves(60, true, (CastlingInfo)0b1111), 0x2c38000000000000LL);
    EXPECT_EQ(board.getKingMoves(60, true, (CastlingInfo)0b1011), 0x2838000000000000LL);
    EXPECT_EQ(board.getKingMoves(60, true, (CastlingInfo)0b0011), 0x2838000000000000LL);
}
...
TEST(BoardTest, slidingAttackTest) {
    ...
    EXPECT_EQ(board.slidingAttack(Board::soEastOne, posInd, board.WHITE()), 0x200000000000LL);
    board.setPiece(29, WPawn);
    GTEST_ASSERT_TRUE(checkBoard(board.slidingAttack(Board::noEastOne, posInd, board.WHITE()), 29));
}
