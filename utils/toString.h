...
static std::string completeInfo(const Evaluation& evaluation, unsigned int nodes, unsLL time, unsigned int ttSize) {
    return format("depth {}\nnodes {}\nnps {}\ntime {}\ntt {}\nbest {}",
        evaluation.depth, nodes, (nodes * 1000) / time , time, ttSize, moveToString(evaluation.move));
}
